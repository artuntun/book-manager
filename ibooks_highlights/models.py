...
class Book(object):
    ...
    def write_org(self, path: pathlib.Path) -> None:
        ...
        mod_date_str = mod_date.isoformat()
        # TODO: add mod_dat_str
        # TODO: wrapp all this in a function. Output tree
        tree = Tree()
        tree.create_node(self._title, 'root')
        tree.create_node('vocabulary', 'vocabulary', parent=tree.root)
        tree.create_node('notes', 'notes', parent=tree.root)
        # for anno in self._annotations:
        ...
        #         if tree.get_node(word): # if the word is repeated
        #             repeated_node = tree.get_node(anno.selected_text)
        #             repeated_node.data['book_examples'].append(anno.represent_text)
        #         else:
        #             # word_info.download_audio(path.joinpath('downloads'))
        #             tree.create_node(anno.selected_text,
        #                             anno.selected_text,
        #                             parent='vocabulary',
        #                             data=word_info.dict())
        #     else:
        #         tree.create_node(anno.selected_text, parent='notes')
        # fn = path / self._filename
        # fn = str(fn)[:-3] + '.org'
        # text = render_org_tree(tree, tree['root'])
        # with open(fn, 'w') as f:
        #     f.write(text)
...
class BookList(object):
    ...
    def write_modified(self, path: pathlib.Path=None, 
                       force: bool=False, file_type: str = 'org') -> None:
